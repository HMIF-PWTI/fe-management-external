...
import { getInfoKp } from "@/service/InfoKp";
import { InfoKp as InfoKpType } from "@/utils/interface";
import { useEffect, useState } from "react";

const InfoKp = () => {
 const [kpData, setKpData] = useState<InfoKpType[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getInfoKp();
